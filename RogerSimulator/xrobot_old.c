...
void evalManager() {
  ...
  if (evalType == PONG) {
    ...
    int fault1 =
      pong_fault_detection(MIN_X, MAX_X, MIN_Y, MAX_Y, middle,
		       ...
		       objects_total[ARM2].position[Y], 
		       R_TACTILE, 1, pong_evaluator);
    int fault2 = 
      pong_fault_detection(MIN_X, MAX_X, MIN_Y, MAX_Y, middle,
    ...
    if (!fault1 && !fault2) {
      ...
      if (pong_goal_detection(MIN_X, MAX_X, ball_x, middle,
			      R_GOAL, DT, &pong_evaluator)) {
