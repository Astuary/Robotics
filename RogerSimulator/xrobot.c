...
// a diagnostic tool to illustrate a trajectory
History history[MaxRobotNum][MAX_HISTORY];
...
// for a single robot in the DEVELOPMENT environment?
Base mobile_base_home;               
Eye eyes_home[NEYES];
Arm arms_home[NARMS][NARM_FRAMES];

// for multiple possible robots in the ARENA environment
...
// THIS SHOULD BE CLEANED OUT---ONE OBJECT (THE RED CIRCLE) FOR NOW
...
int draw_visual = TRUE; // enables user specified graphics for rendering
...
int evalType = -1; // game id initialized to -1 (no game)
PongEval pong_evaluator;  // defined in include/evaluations.h
...
void evalManager() {
  ...
  if (evalType == PONG) {
    ...
    if (!fault1 && !fault2) {
      ...
      if (pong_goal_detection(MIN_X, MAX_X, ball_x, middle, R_GOAL, DT,
			      &pong_evaluator)) {
...
int rogerID;
{
  ...
  for (i=0; i<NPIXELS; i++) {
    RogersBodys[rogerID].eyes[LEFT].image[i] = 
      RogersBodys[rogerID].eyes[RIGHT].image[i] = 100;
  }
...
//    for (k=0; k<numRoger; ++k) {
    k = rogerID;
      ...
      sim_fwd_kinematics(LEFT, RogersBodys[k].arms[LEFT][1].theta,
			 RogersBodys[k].arms[LEFT][2].theta, &x, &y);
      ...
      sim_fwd_kinematics(RIGHT, RogersBodys[k].arms[RIGHT][1].theta,
			 RogersBodys[k].arms[RIGHT][2].theta, &x, &y);
      vobject[o_index].dx = x - RogersBodys[rogerID].eyes[eye].position[X];
      vobject[o_index].dy = y - RogersBodys[rogerID].eyes[eye].position[Y];
      vobject[o_index].radius = R_JOINT;
      vobject[o_index++].color = ARM_COLOR;
//    }
    ...
    // grid points at the next empty element of the range array
    for (i = 0; i<NBINS; ++i) {
