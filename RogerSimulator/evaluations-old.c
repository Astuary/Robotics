...
PongEval* pong_evaluator;
{
  if (pong_evaluator->goal_1 == MAX_SCORE || 
      pong_evaluator->goal_2 == MAX_SCORE) {
    if (pong_evaluator->goal_1 == MAX_SCORE) {
      printf("Roger 1 WINS\n");
    }
...
int pong_fault_detection(min_x, max_x, min_y, max_y, middle, 
                         ...
                         arm1_pos_x, arm1_pos_y,
                         arm2_pos_x, arm2_pos_y,
                         hand_radius, roger_id, pong_evaluator) 
...
PongEval* pong_evaluator;
{
  ...
  if ((base_pos_x + base_radius) > max_x || 
      (base_pos_x - base_radius) < min_x ||
      (base_pos_y + base_radius) > max_y ||
      (base_pos_y - base_radius) < min_y) {
  ...
  if ((arm1_pos_x + hand_radius) > max_x || 
      (arm1_pos_x - hand_radius) < min_x ||
      (arm1_pos_y + hand_radius) > max_y ||
      (arm1_pos_y - hand_radius) < min_y ||
      (arm2_pos_x + hand_radius) > max_x ||
      (arm2_pos_x - hand_radius) < min_x ||
      (arm2_pos_y + hand_radius) > max_y ||
      (arm2_pos_y - hand_radius) < min_y) {
